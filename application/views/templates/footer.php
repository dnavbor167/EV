...
<footer>
    <div id="evFooter">
        ...
        <div>
            <h3>
                <?= strtoupper($this->lang->line('eternals')); ?>
            </h3>
            <h3 id="vibesFooter">
                <?= strtoupper($this->lang->line('vibes')); ?>
            </h3>
        </div>
    ...
    <div id="evContact">
        <h3>
            <?= strtoupper($this->lang->line('contact')); ?>
        </h3>
    ...
    <div id="homeServiceFooter">
        <p><a href="<?= $is_home ?>">
                <?= strtoupper($this->lang->line('home')); ?>
            </a></p>
        <p><a href="<?= site_url('Dashboard/paymentsPlans'); ?>">
                <?= strtoupper($this->lang->line('plans')); ?>
            </a></p>
        <p><a href="#">
                <?= strtoupper($this->lang->line('services')); ?>
            </a></p>
        <p><a href="#">
                <?= strtoupper($this->lang->line('about')); ?>
            </a></p>
    </div>
    ...
    <div id="subscribeFooter">
        <div id="privacyTerms">
            <p><a href="#">
                    <?= strtoupper($this->lang->line('privacyPolicy')); ?>
                </a></p>
            <p><a href="#">
                    <?= strtoupper($this->lang->line('termsServices')); ?>
                </a></p>
        </div>

        <form action="#" method="post" id="newsLetter">
            <label for="emailNews">
                <?= strtoupper($this->lang->line('newsLetterText')); ?>
            </label>
            <input placeholder="<?= $this->lang->line('newsLetterPlace'); ?>" type="email" name="emailNews"
                id="emailNews">
            <button type="submit" id="btnNewsletter">
                <?= strtoupper($this->lang->line('newsLetterButton')); ?>
            </button>
        </form>
    </div>
</footer>
...
<script>
    ...
    <?php if ($this->session->userdata('is_logged_in')) { ?>
        ...
        $('#confiUserImg').on('click', function (e) {
            e.preventDefault();
            e.stopPropagation();

            // Cerrar el menú de grupo si está abierto
            if ($('#configGroup').is(':visible')) {
                $('#configGroup').stop(true, true).slideUp(300);
            }
        ...
        $('#configGroupImg').on('click', function (e) {
            e.preventDefault();
            e.stopPropagation();
            ...
            // Toggle del menú de grupo
            $('#configGroup').stop(true, true).slideToggle(300);
        });

        // Cerrar ambos menús si se hace clic fuera
        $(document).on('click', function () {
            $('#configUser, #configGroup').filter(':visible').stop(true, true).slideUp(300);
        });

        // Evitar que el clic dentro del menú cierre los paneles
        $('#configUser, #configGroup').on('click', function (e) {
            e.stopPropagation();
        });

        // Cerrar ambos menús al redimensionar la ventana
        $(window).on('resize', function () {
            $('#configUser, #configGroup').filter(':visible').stop(true, true).slideUp(300);
        });

        //Modal de confirmación para eliminarse de un grupo
        $('#exitGroupMenu').on('click', function (e) {
            e.preventDefault();
            $('#exitGroup').fadeIn();
        });
    ...
    <?php if (empty($this->session->userdata('groups')) && $this->session->userdata('is_logged_in')): ?>
        // Mostrar modal al hacer clic en enlaces que requieren login
        $('.requires-login').on('click', function (e) {
            e.preventDefault();
